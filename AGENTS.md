...
- 宝物庫の中身はアイテム個体ではなくプレイヤーUUIDへ紐づけ、OverworldのSavedDataに保存する。
- 1プレイヤーの最大収蔵数は10,000本、同一論理スタックは最大1,028本とする。これらの上限を無制限化しない。
- 同一スタック判定はアイテムと全Data Componentの完全一致で行い、耐久値・名前・エンチャント・SlashBlade内部値・VenusMod強化値が異なる刀をまとめない。
- 通常のItemStack最大スタック数を変更しない。代表ItemStack 1本と論理countとして保存する。
- 自動回収は `ItemEntityPickupEvent.Pre` で通常Inventory追加より前に行い、SavedDataへの保存成功後だけ地面のItemEntityを減らす。失敗時に刀を消失させない。
- SlashBladeは任意依存を維持し、宝物庫機能からSlashBladeクラスを直接参照しない。
