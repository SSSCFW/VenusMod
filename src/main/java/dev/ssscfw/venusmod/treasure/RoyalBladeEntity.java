...
public final class RoyalBladeEntity extends Projectile {
    ...
    public void launch(Vec3 target) {
        ...
        if (direction.lengthSqr() < 1.0E-6) direction = getOwner() == null ? new Vec3(0, 0, 1) : getOwner().getLookAngle();
    ...
    private void follow(LivingEntity owner) {
        ...
        int row = formationSlot / 8;
        double angle = Math.PI * ((formationSlot % 8) + 0.5) / 8.0;
        double radius = 1.45 + row * 0.85;
        Vec3 position = owner.getEyePosition().subtract(forward.scale(1.6 + row * 0.12))
                .add(right.scale(Math.cos(angle) * radius)).add(0, Math.sin(angle) * radius + 0.25, 0);
        setPos(position);
    ...
    @Override public void tick() {
        ...
        if (level().isClientSide) {
            if (launched()) {
                ...
                if ((tickCount & 1) == 0) level().addParticle(ParticleTypes.END_ROD, getX(), getY(), getZ(), 0, 0, 0);
            }
            return;
        }
        if (!(getOwner() instanceof LivingEntity owner) || !owner.isAlive() || owner.isRemoved()
                || owner.level() != level() || owner.isSpectator()) { discard(); return; }
        if (!launched()) {
            // 管理側の後始末が行われなくても、無期限に残さない。
            if (tickCount >= TreasureRules.PREPARE_TICKS) discard();
            else follow(owner);
            return;
        }
        if (++flightTicks >= TreasureRules.FLIGHT_TICKS) { discard(); return; }
        Vec3 start = position();
    ...
    /** PvP設定・同盟・自分のペットを尊重する。アイテムや額縁等は攻撃対象にしない。 */
    ...
    private void burst(LivingEntity owner, Entity directTarget) {
        ...
        discard(); // ダメージイベントから再入されても爆発は1回だけ。
        ...
        // Level.explodeは呼ばない。ブロック破壊・着火・ドロップ破壊を発生させない範囲爆発。
        for (LivingEntity target : level.getEntitiesOfClass(LivingEntity.class, new AABB(center, center).inflate(radius),
            ...
            float damage = direct ? TreasureRules.DAMAGE : (float) (TreasureRules.DAMAGE * 0.5 * (1 - distance / radius));
    ...
    @Override protected void readAdditionalSaveData(CompoundTag tag) {
        ...
        discard(); // 古い保存データやコマンドによって投影が復元されても実体化させない。
    }
}
